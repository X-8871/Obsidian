...
- **当前核心文档**：
  ...
  - **各工具专属行为档案 (tool_profiles)**：
    - [[ANTIGRAVITY]] —— Antigravity 专属：**网页操作后必须强制显式调用 `agent_browser_close` 彻底关闭浏览器**、Subagent 调度与规划模式对齐。
    - [[CODEX]] —— Codex 专属：Computer-Use 与 Node 运行时管道释放、沙箱权限、Ponytail 渐进重构规范。
    - [[OPENCODE]] —— OpenCode 专属：飞书生态 API 凭据安全与频控、AutoGLM 多端生成缓存清理、本体图谱同步规范。
...
- **当前核心文档**：
  - [[Person_WorkBeach统一工作台_维护说明]] —— 以 WorkBench 为核心的五模块统一入口、平台能力、单仓库治理、独立数据边界、启动测试与故障隔离总手册。
  - [[lab-coach_科研学习教练Agent_维护说明]] —— lab-coach 科研教学 Agent 的双入口架构、SQLite 状态机、CLI 命令集、苏格拉底红线与 NanoGPT AX7020 移植案例。
  ...
  - [[Lzheng Fitness Web 维护说明]] —— Lzheng Fitness 本地个人健身工作台的路径、运行方式、接口契约、安全红线与后续 API 接入待办。
  - [[PlanMind计划管理_维护说明]] —— PlanMind 的 Next.js 架构、Obsidian `08-PlanMind` 数据边界、独立启动和统一平台 AI 适配。
  - [[InkOS_mNOVA小说Agent_维护说明]] —— InkOS/mNOVA 小说 Agent 的源码与运行目录、默认简洁爽文架构、启动测试指令、架构红线及 Gate 1 当前状态。
...
- **当前核心文档**：
  - [[lab-coach_苏格拉底科研教学闭环SOP]] —— 涵盖五步教学闭环、L0~L5 提示分级、实验单生成、报告评审打回与进阶挑战的完整 SOP。
  - [[Obsidian课程资料整理SOP]] —— 试卷切图、智能纠偏、长卷组装与双链整理规范。
  - [[大文件清单式原子同步验收SOP]] —— 覆盖 A/B 模式、流式大文件、原子 revision、故障注入、权限与资源验收的通用流程。
  - [[mNOVA_Agent能力验证与防幻觉验收SOP]] —— 默认简洁模式下的四章实跑、契约哈希、状态结算、伏笔唯一性与失败安全验收流程。
...
graph TD
    ...
    R --> R3["📁 tool_profiles<br/>([[ANTIGRAVITY]] / [[CODEX]] / [[OPENCODE]])"]
    O --> O1["[[常用开发环境与工具链]]"]
    O --> O2["[[Git与GitHub连接配置]]"]
    O --> O3["[[云服务器连接与运维_模板]]"]
    ...
    P --> P00["[[Person_WorkBeach统一工作台_维护说明]]"]
    P --> P0["[[lab-coach_科研学习教练Agent_维护说明]]"]
    P --> P1["[[Obsidian_知识库运维与维护说明]]"]
    P --> P2["[[重点项目维护说明_模板]]"]
    P --> P3["[[Minecraft_Fabric私服与AI新闻机器人_维护说明]]"]
    P --> P4["[[Mouse_Workbench与Obsidian同步服务_维护说明]]"]
    P --> P5["[[InkOS_mNOVA小说Agent_维护说明]]"]
    P --> P6["[[PlanMind计划管理_维护说明]]"]
    P --> P7["[[Lzheng Fitness Web 维护说明]]"]
    W --> W0["[[lab-coach_苏格拉底科研教学闭环SOP]]"]
    W --> W1["[[Obsidian课程资料整理SOP]]"]
    W --> W2["[[大文件清单式原子同步验收SOP]]"]
    W --> W3["[[mNOVA_Agent能力验证与防幻觉验收SOP]]"]
    V --> V1["[[私密凭据安全说明与模板]]"]

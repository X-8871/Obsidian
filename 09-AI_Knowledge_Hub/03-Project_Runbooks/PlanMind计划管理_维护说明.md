...
> [!INFO] 项目定位
> - **项目物理绝对路径**：`C:\Users\22061\Desktop\Person_WorkBeach\apps\plan-web`
...
| 数据事实源 | 用户配置的 Obsidian Vault，默认目录 `08-PLAN` |
...
- 目标、任务、历史计划和同步内容只读写用户配置的 Obsidian `08-PLAN`；
...
```powershell
Set-Location 'C:\Users\22061\Desktop\Person_WorkBeach\apps\plan-web'
npm install
npm run dev
```
...
2. 备份 Obsidian `08-PLAN`，保持内部双链和相对路径；
3. 不把 API Key、Vault 私有正文或本机绝对配置提交到 Git；
4. 恢复时只恢复 PlanMind 自己的配置与 `08-PLAN`，不得覆盖平台数据库或其他模块数据。
...
1. **禁止把计划数据迁入平台 SQLite**：Obsidian `08-PLAN` 继续是计划领域事实源。
...
8. **禁止修改其他模块配置来修复 PlanMind**：故障只在 `apps/plan-web`、平台适配或用户指定的 `08-PLAN` 范围内处理。
...
| 计划列表为空 | Vault 路径错误、`08-PLAN` 不存在或文件解析失败 | 核对本机配置和目录权限，禁止先创建空数据覆盖原目录 |

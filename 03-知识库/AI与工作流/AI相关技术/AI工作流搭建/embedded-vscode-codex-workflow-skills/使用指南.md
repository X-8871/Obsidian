...
status: active
---
# 嵌入式 VS Code Codex 工作流使用指南

这套工作流用于让新嵌入式项目拥有长期记忆、任务边界、证据规则和接力能力。

它适合 MCU、SoC、固件、传感器、串口日志、板级调试、本地构建、烧录和人工验证类项目。不建议用于纯前端、纯后端或内容写作项目。
...
`workflow` 只做一次性初始化。它会：

- 检查项目是否已经初始化过。
- 询问项目目标、阶段、芯片/开发板、真机条件、开发环境模式、禁区文件和第一轮任务。
- 读取少量项目证据。
- 生成 `AGENTS.md`、`docs/ai_context/*` 和项目内 workflow skills。
- 不修改业务代码。
- 不运行构建、烧录或硬件验证，除非用户明确要求。

如果项目里已经存在 `docs/ai_context/WORKFLOW_INIT_STATE.md`，说明这个项目已经初始化过，不要再次执行。
...
```text
使用 $embedded-project-relay 接手项目。
...
调试硬件、驱动、GPIO、外设、串口、启动、状态机或构建问题时使用：
